  ...
  return (
    <header 
      className={`fixed w-full z-30 transition-all duration-300 ${
        scrolled ? 'py-3 bg-white shadow-sm' : 'py-5 bg-transparent'
      }`}
    >
        ...
        <nav className="hidden md:flex items-center space-x-8">
          {navItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) => 
                `text-sm uppercase tracking-wider hover:text-arch-gray transition-colors ${
            ...
            <div className="flex-1 flex flex-col justify-center items-center space-y-8">
              {navItems.map((item) => (
                <NavLink
                  ...
                  onClick={toggleMenu}
                  className={({ isActive }) => 
                    `text-2xl uppercase tracking-wider hover:text-arch-gray transition-colors ${
                      isActive ? 'font-medium' : 'font-normal'
                    }`
                  }
                >
